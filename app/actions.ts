...
export async function generateImage(
	...
	if (style === "formal") {
		...
		prompt =
			"A professional smart-casual vertical portrait of the same person from the input image, but clearly a different photo. Maintain the exact facial identity and realistic skin texture. The person is wearing smart-casual professional clothing such as a well-fitted shirt, clean knit sweater, or casual blazer in neutral tones. Modern and relaxed professional style suitable for both men and women. Upper body portrait, natural posture, confident and approachable expression, looking at the camera. Clean contemporary background with soft gradients or minimal office-like setting. Soft diffused professional lighting with gentle shadows and natural color tones. Photorealistic, high detail, sharp focus on facial features. Change clothing, background, lighting and pose compared to the original image. Avoid recreating the original framing. No particles, no artifacts, no distortions, no over-smooth or plastic skin, no exaggerated colors, no lens flares, no text, no watermarks.";
